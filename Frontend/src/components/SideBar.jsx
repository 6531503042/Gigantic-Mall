...
const Sidebar = ({ children }) => {
    ...
    const [isWide, setIsWide] = useState(window.innerWidth >= 1528);

    const toggle = () => setIsOpen(!isOpen);

    useEffect(() => {
        const handleResize = () => {
            setIsWide(window.innerWidth >= 1528);
        };
        ...
        <div className='sidebar-container'>
            <div className='sidebar' style={{ width: isOpen ? "370px" : "155px" }}>
                <div className='top-section'>
                    <h1 className='logo_admin' style={{ display: isOpen && !isWide ? "block" : "none" }}>
                ...
                {menuItem.map((item, index) => (
                    <NavLink to={item.path} key={index} className={`link ${item.className || ''}`} activeClassName="active">
                        <a className='link-line'>
                            ...
                            <div className='link_text' style={{ display: isOpen && !isWide ? "block" : "none" }}>{item.name}</div>
                        </a>
                    </NavLink>
                ))}
            </div>
